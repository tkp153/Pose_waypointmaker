...
bool rtabmap_ros__msg__odom_info__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // local_bundle_models
    ...
    if (!rtabmap_ros__msg__CameraModels__Sequence__init(&(ros_message->local_bundle_models), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create rtabmap_ros__msg__CameraModels__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    rtabmap_ros__msg__CameraModels * dest = ros_message->local_bundle_models.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!rtabmap_ros__msg__camera_models__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
PyObject * rtabmap_ros__msg__odom_info__convert_to_py(void * raw_ros_message)
{
  ...
  {  // local_bundle_models
    ...
    rtabmap_ros__msg__CameraModels * item;
    for (size_t i = 0; i < size; ++i) {
      item = &(ros_message->local_bundle_models.data[i]);
      PyObject * pyitem = rtabmap_ros__msg__camera_models__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }
